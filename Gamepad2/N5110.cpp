...
void N5110::drawLine(int const x0,
                     int const y0,
                     int const x1,
                     int const y1,
                     int const type)
{
    ...
    if ( abs(x_range) > abs(y_range) ) {
        ...
        int const start = x_range > 0 ? x0:x1;
        int const stop =  x_range > 0 ? x1:x0;
        ...
        // ensure we loop from smallest to largest or else for-loop won't run as expected
        int const start = y_range > 0 ? y0:y1;
        int const stop =  y_range > 0 ? y1:y0;
